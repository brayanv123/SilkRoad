...
public class Robot
{
     ...
     */
    public Robot(Rectangle rectangle, int initialLocation){
        size = 5;
        xPosition = rectangle.getXPosition();
        yPosition = rectangle.getYPosition();
        actualX = rectangle.getXPosition();
        actualY = rectangle.getXPosition();
        createRobot();
        actualTenges = 0;
        rInit = rectangle;
    ...
    public void moveTo(Rectangle rectangle){
        ...
        actualX = rectangle.getXPosition();
        actualY = rectangle.getYPosition();
        
        int armX = actualX - percentage(size, 0.3);
        int armY = actualY - size;

        int legX = actualX + percentage(size, 0.2);
        int legY = actualY;
        
        arms.moveTo(armX, armY);
        legs.moveTo(legX, legY);
        head.moveTo(actualX, (actualY - (2 * size)));
        body.moveTo(actualX, (actualY - size));
    }
